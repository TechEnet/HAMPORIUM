...
/*
 * This automatically uses order.taxableAmount.
 * After the partner promo changes in order.controller.js, taxableAmount is
 * already reduced by the customer promo BEFORE this function runs. Therefore
 * partner commission is calculated on the actual discounted taxable value.
 */
export const ensureCommissionForPaidOrder = async (
  order,
  { actorId = null } = {}
) => {
  if (!order?.partnerAttribution?.partner) return null;

  const existing = await Commission.findOne({ order: order._id });
  if (existing) return existing;

  const partner = await Partner.findById(
    order.partnerAttribution.partner
  ).select("+defaultCommissionRate status owner");

  if (!partner) return null;

  let rate = Number(partner.defaultCommissionRate || 0);

  if (order.partnerAttribution.project) {
    const project = await PartnerProject.findById(
      order.partnerAttribution.project
    ).select("+commissionRateOverride");
  ...
  const commission = await Commission.create({
    ...
    partner: partner._id,
    referral: order.partnerAttribution.referral || null,
    project: order.partnerAttribution.project || null,
    showcase: order.partnerAttribution.showcase || null,
    order: order._id,
    ...
    history: [
      {
        status: COMMISSION_STATUS.ORDER_CONFIRMED,
        changedBy: actorId,
        note: "Paid order automatically attributed to partner.",
      },
    ],
  });
